...
final class LaunchAtLoginServiceTests: XCTestCase {
    func testEnableWritesLaunchAgentPlist() throws {
        ...
        let plistURL = tempRoot
            .appendingPathComponent("Library/LaunchAgents/com.aibalancemonitor.app.launchatlogin.plist")
        XCTAssertTrue(FileManager.default.fileExists(atPath: plistURL.path))
        let data = try Data(contentsOf: plistURL)
        let plist = try XCTUnwrap(
            PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any]
        )
        XCTAssertEqual(plist["Label"] as? String, "com.aibalancemonitor.app.launchatlogin")
