...
final class RelayProviderTests: XCTestCase {
        ...
        let descriptor = ProviderDescriptor(
            ...
            openConfig: OpenProviderConfig(
                ...
                accountBalance: RelayAccountBalanceConfig(
                    enabled: true,
                    auth: AuthConfig(kind: .bearer, keychainService: "AIBalanceMonitor", keychainAccount: "open.ailinyu.de/session-cookie"),
                    authHeader: "Cookie",
                    authScheme: "",
                    requestMethod: "GET",
                    requestBodyJSON: nil,
    ...
    func testUnknownLegacyRelayFallsBackToGenericManifestAndKeepsAccount() {
        let descriptor = ProviderDescriptor(
            ...
            pollIntervalSec: 60,
            threshold: AlertRule(lowRemaining: 10, maxConsecutiveFailures: 2, notifyOnAuthError: true),
            auth: AuthConfig(kind: .bearer, keychainService: "AIBalanceMonitor", keychainAccount: "relay.example/sk-token"),
            baseURL: "https://relay.example.com",
            openConfig: OpenProviderConfig(
                tokenUsageEnabled: true,
                accountBalance: RelayAccountBalanceConfig(
                    enabled: true,
                    auth: AuthConfig(kind: .bearer, keychainService: "AIBalanceMonitor", keychainAccount: "relay.example/system-token"),
    ...
    func testMakeOpenRelayPreservesExplicitPreferredTemplate() {
        let descriptor = ProviderDescriptor.makeOpenRelay(
            ...
            keychainService: "AIBalanceMonitorTests"
        )
