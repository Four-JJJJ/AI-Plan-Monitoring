...
V2.1.0 的重点是补齐使用统计工作台，继续拆分核心运行时，并优化设置页入口和侧边栏资源。

本次更新集中在六件事：

| 方向 | 改进 |
| --- | --- |
| 使用统计 | 新增设置页“使用统计”入口，汇总请求数、成功数、输入 / 输出 / 缓存 Token 与趋势图 |
| 数据来源 | 支持聚合 Codex、Claude、Kimi 本地用量与 CC Switch 代理日志，并按来源优先级去重 |
| 统计缓存 | 增加使用统计快照缓存、文件指纹校验和按时间窗口刷新，减少重复扫描本地日志 |
| 设置页入口 | 设置窗口默认打开“使用统计”，侧边栏新增“请我喝咖啡”入口与二维码支持 |
| 图标资源 | 替换使用统计和赞赏菜单的未选中 / 选中图标，统一为更克制的 14x14 资源 |
| 架构拆分 | 继续把 AppViewModel、Provider 模型、菜单栏展示、设置页和运行时服务拆到更小模块 |
