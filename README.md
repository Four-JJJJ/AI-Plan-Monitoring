...
1. Claude 官方监控支持单独指定菜单栏展示账号，顶部 status bar 与菜单卡片现在会统一跟随这个已选账号，不再默认跟着当前系统正在使用的 Claude 凭证跑。
2. 如果本机当前 Claude 凭证只是 `llm proxy / inference-only`，会自动从官方监控卡片里隐藏，不再长期显示空数据或报错卡片干扰多账号监控。
3. 新增 `Mimo Token Plan` 模板，支持把小米 mi'mo 的 Token Plan 以套餐百分比卡形式展示，并补齐菜单栏/设置页的套餐名、有效期和相关回归测试。
