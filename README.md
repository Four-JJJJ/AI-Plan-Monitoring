...
1. 新增 Codex/Claude OAuth 账号导入路径，保持现有设置页结构不变，支持最小增量接入。
2. Codex OAuth 登录流程支持浏览器回调失败后自动回退 Device Auth，提升导入成功率。
3. 修复账号去重场景下的误覆盖问题：同账号导入会优先合并到已有档案，不再错误覆盖其他账户。
4. Claude 监控卡片扩展为 4 指标展示，账号展示与状态信息更完整。
5. 用量偏好设置扩展到更多 provider，官方来源展示与设置一致性进一步提升。
