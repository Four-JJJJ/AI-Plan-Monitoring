...
1. 新增 Claude 本机多账号管理：支持通过 `CLAUDE_CONFIG_DIR` 或手动粘贴 `.credentials.json` 导入，并可在菜单中一键切换账号。
2. Codex 账号识别升级：引入 tenant/principal/identity 身份键，解决“同邮箱不同 Team”被误合并的问题。
3. Gemini 兼容性增强：适配新版 Gemini CLI 的凭证与配额结构（含 buckets），项目识别和额度解析更稳定。
4. 菜单栏体验优化：卡片区支持超长滚动、状态图标与文字对齐优化、弹层定位和背景一致性提升。
5. 设置页重构：本地扫描/重置区与官方配置区交互统一，反馈更清晰；Relay 凭证提示改为“按请求头值填写”，降低误填。
6. 安全与稳定性提升：钥匙串写入区分交互与静默场景，减少打断；补充 Claude/Codex/Gemini 相关单测。
