...
1. GitHub Copilot 官方来源补齐自动鉴权发现链路，默认会按顺序识别环境变量、Copilot CLI 与 GitHub CLI 登录态，减少已登录却显示 Unauthorized 的情况。
2. GitHub Copilot 认证失败时改成更明确的原因提示，区分凭证失效和权限/订阅不可用，方便直接排查。
3. 同步刷新安装包、更新清单和 GitHub Release 资源，保证新版本下载与应用内更新提示一致。
