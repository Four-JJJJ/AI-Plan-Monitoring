...
1. 新增本地使用趋势：Codex、Claude、Kimi 支持读取本机日志，展示今日、昨日、近 30 天用量，以及 24 小时 / 7 天趋势图。
2. 支持按账号查看趋势：Codex、Claude 可在“全量 / 按账号”之间切换，更适合多账号、多 Team 场景。
3. 菜单栏展示增强：模型卡新增套餐类型展示；Codex 同邮箱多 Team 会用 Team A / Team B 区分，避免看不清是哪一个团队。
4. 设置页体验优化：左侧模型列表支持拖拽排序，官方模型详情区新增关键数据和使用趋势，配置查看更直观。
5. Kimi / Gemini 解析增强：保留原始模型维度额度数据，重置时间、模型 ID、套餐信息展示更稳定。
6. 减少凭证读取打扰：浏览器凭证和钥匙串读取增加短缓存，降低频繁弹授权或重复扫描的概率。
7. 倒计时显示统一：菜单栏和设置页统一使用 HH:MM:SS 格式，过期窗口自动归零，显示更一致。
