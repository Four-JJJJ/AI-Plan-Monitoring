...
1. 新增第三方 API 余额动态基线追踪，余额波动展示更平滑，异常跳变更易识别。
2. 设置页与菜单内容样式统一，并优化设置窗口/菜单视图尺寸与排版一致性。
3. 修复 Codex 自动采集保留策略与鉴权路径解析问题，多账号状态读取更稳定。
4. 优化 Claude 本地用量链路的缓存与预取策略，降低长期运行内存增长风险。
