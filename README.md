...
1. 升级应用内自更新链路：从版本检测、下载校验、解压准备到覆盖安装与重启，形成完整流程，并补充失败重试与安装缓冲状态展示。
2. 更新源从 GitHub Release API 调整为 `latest.json` 元数据方案，支持 ZIP/DMG 资产、发布时间、校验值等信息，更新可靠性更高。
3. 状态栏支持多模型同时展示，可在设置里选择多个 Provider，并新增两种展示样式：`图标+百分比`、`柱状图+文字`。
4. 状态栏渲染重构为独立渲染器（`StatusBarDisplayRenderer`），并优化刷新策略与低功耗轮询，减少无效刷新，改善倒计时与布局稳定性。
5. 官方模型配置新增“在菜单栏显示套餐类型”能力，Codex/Claude/Kimi/Gemini 的展示信息更完整。
6. Generic New API 模板重做：默认改为余额通道优先，补充 `userId(New-Api-User)` 必填，额度字段按美元单位换算，兼容配置与测试同步更新。
7. Relay 解析能力增强：表达式新增 `div(...)`，并补充 `requestCount/rawUsedQuota` 等元数据，账号通道 `note` 信息更可读。
8. Codex 桌面端重启服务重构：支持自定义应用匹配与强制终止策略，提升“检测到运行中后重启”的可控性。
