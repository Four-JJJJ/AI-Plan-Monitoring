...
1. 覆盖安装或异常恢复后，如果主配置和 backup 配置失效，状态栏单模型/多模型展示、展示样式等配置现在会从 `config.recovery.json` 一并恢复，不再只剩官方监控启用态。
2. 强化 Keychain 安全存储恢复：即使之前因为非交互读取 miss 导致看起来像“没同步”，现在也会重新装载可读 vault，减少覆盖安装后凭证状态误判。
3. 同步刷新安装包、更新清单和 GitHub Release 资源，保证当前 `v1.8.4` 下载内容和 `main` 一致。
