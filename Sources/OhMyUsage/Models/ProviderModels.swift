...
extension ProviderDescriptor {
    ...
    var officialRelayAdapterID: String? {
        ...
        return RelayAdapterRegistry.shared.manifest(for: normalizedBaseURL).id
    }
    ...
    static let officialRelayDefaultProviderIDs: Set<String> = [
        "moonshot-official",
        "minimax-official",
        "deepseek-official",
    ...
    static let officialRelayDefaultProviderOrder: [String] = [
        "moonshot-official",
        "minimax-official",
        "deepseek-official",
        "xiaomi-mimo-official"
    ...
    private static func defaultRelayBalanceAccount(
        ...
        let isOfficialRelayBaseURL = officialRelayDefaultBaseURL(adapterID: adapterID).map {
            normalizeRelayBaseURL($0).lowercased() == normalized.lowercased()
        } ?? false
        switch adapterID {
        case "ailinyu":
        ...
        case "xiaomimimo" where isOfficialRelayBaseURL,
             "xiaomimimo-token-plan" where isOfficialRelayBaseURL:
            return "platform.xiaomimimo.com/session-cookie"
        case "moonshot" where isOfficialRelayBaseURL:
            return "platform.moonshot.cn/auth_token"
        case "deepseek" where isOfficialRelayBaseURL:
            return "platform.deepseek.com/system-token"
        case "minimax" where isOfficialRelayBaseURL:
            return "platform.minimaxi.com/system-token"
        default:
...
extension AppConfig {
    func migratedWithSiteDefaults() -> AppConfig {
        ...
        let officialRelayProviderIDMap = migrated.migrateOfficialRelayProvidersToStableIDs()
        ...
        migrated.migrateOfficialRelayDefaultOrderAfterKimiIfNeeded()
    ...
    private mutating func migrateOfficialRelayDefaultOrderAfterKimiIfNeeded() {
    ...
    private func shouldMigrateOfficialRelayDefaultOrder(providerIDs: [String], relayIDs: [String]) -> Bool {
    ...
    private mutating func migrateOfficialRelayProvidersToStableIDs() -> [String: String] {
        ...
        for provider in providers {
            ...
            if let existingIndex = migratedProviders.firstIndex(where: { $0.id == targetID }) {
                migratedProviders[existingIndex] = Self.mergedOfficialRelayProvider(
...
private extension ProviderDescriptor {
    func migratedSiteDefaults(from defaults: ProviderDescriptor) -> ProviderDescriptor {
        ...
        if copy.isRelay, defaults.isRelay {
            return copy.migratedRelayDefaults(from: defaults)
        }
    ...
    func migratedRelayDefaults(from defaults: ProviderDescriptor) -> ProviderDescriptor {
        var copy = self
        copy.family = defaults.family
